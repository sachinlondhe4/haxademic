...
implements IVizElement {
	...
	public void init() {
		...
		_meshPool.addMesh( "MUSIC_NOTE", MeshUtil.meshFromImg( p, "../data/images/music.gif", 1f ), 40f * scaleMult );
...
//		_meshPool.addMesh( "CDW_LOGO", MeshUtil.meshFromSVG( p, "../data/svg/create-denver-logo.svg", -1, 2, 0.6f ), 1 );
		...
		for( int i=0; i < _meshPool.size() - 1; i++ ) {
	...
	public void selectNewModel() {
		_meshIndex++;
		if( _meshIndex >= _meshPool.size() - 1 ) _meshIndex = 0;
